...
本文件保留 RC 的轻量搜索方案。全局调度由 [Kernel](../SKILL.md) 负责；以下路由箭头只表示向 Kernel 返回的建议，不建立第二个 Router。
...
→ 返回 Kernel；候选需要比较时才建议 `REASONING`，可以直接测试或行动时无需先比较。
...
> **现在是否已有足够的、真正不同且值得继续检查的候选？**

3–5 个是参考规模，不是配额；两个候选已能支持测试或行动时也应退出。
...
仅在继续搜索仍有价值时才允许升级；已饱和或瓶颈改变时返回 Kernel。
...
不要默认 Level 5。各级是逐步增加深度的可选工具，不要求逐级全部执行；选与当前搜索缺口直接相关的一项即可。
...
# 17. INDEPENDENCE IN DEEP MODE

Deep Mode 同样遵循第 6 节的独立生成规则，不新增一套互动流程。
...
去重合并在第 9 节 Cluster 内完成，不额外运行一次流程：删去名称和包装后机制相同的候选应合并。
...
只给 Kernel 一个有依据的建议，不自行启动下个引擎。例如：
...
Brainstorm 默认甚至不需要真人。若使用真人载体，只读取 [Deliberation 的真人与真实性规则](deliberation-engine.md#6-cognitive-seats-与真人载体)。
...
默认预算遵循第 5–7 节 Light Mode；扩展依据第 10–11 节，只处理尚未解决的搜索缺口。用户要求深度不等于自动增加字数或候选数量。

---

# 40. QUALITY GATE

Brainstorm 结束前从以下六个角度中只检查当前相关项，沿用 Kernel 的 What changed 检查，不增加完整必跑清单：
...
且满足 Kernel 第 21 节的可逆行动条件，则返回 Kernel 选择：
